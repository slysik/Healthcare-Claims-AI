...
if [ ! -f "${DATA_DIR}/sample_benefits_summary.pdf" ]; then
    echo "Generating benefits PDF..."
    cd "${PROJECT_DIR}/backend"
    uv run python "${DATA_DIR}/generate_benefits_pdf.py"
    echo "Generated: ${DATA_DIR}/sample_benefits_summary.pdf"
else
    echo "Benefits PDF already exists: ${DATA_DIR}/sample_benefits_summary.pdf"
fi

echo ""
echo "=== Data Ready ==="
ls -lh "${DATA_DIR}"/sample_claims.csv "${DATA_DIR}"/sample_benefits_summary.pdf
