...
export default function ChatPanel() {
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto">
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-8 space-y-5">
        {messages.length === 0 && !isStreaming && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <Sparkles className="h-16 w-16 text-brand-400/40 mb-4" />
            <h2 className="text-xl font-semibold text-brand-800 mb-2">
              Healthcare Claims AI
            </h2>
            <p className="text-sm text-brand-600/70 mb-6 max-w-md">
            ...
            <div className="flex flex-wrap gap-2 justify-center max-w-2xl">
              {SUGGESTED_QUERIES.map((query) => (
                <button
                  ...
                  className="text-sm px-3 py-1.5 rounded-full border border-brand-200 text-brand-600 shadow-sm ring-1 ring-brand-200 hover:ring-brand-300 hover:shadow-md hover:bg-brand-50 transition-all duration-200"
                >
                  {query}
                </button>
              ))}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            {SUGGESTED_QUERIES.slice(0, 3).map((query) => (
              <button
                key={query}
                onClick={() => handleSuggestionClick(query)}
                className="text-xs px-3 py-1 rounded-full border border-brand-200 text-brand-600 hover:bg-brand-50 transition-all duration-200 whitespace-nowrap flex-shrink-0"
              >
                {query}
              </button>
            ))}
      ...
      {/* Input area */}
      <div className="border-t border-brand-100 bg-white p-3 sm:p-4 shadow-sm">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <textarea
            ...
            className="flex-1 px-4 py-2.5 rounded-lg border border-brand-200 focus:outline-none focus:ring-2 focus:ring-brand-300 focus:border-brand-400 text-sm resize-none max-h-32 min-h-[42px]"
          ...
          {isStreaming ? (
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-lg bg-gradient-to-r from-brand-500 to-brand-600 text-white hover:from-brand-600 hover:to-brand-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-sm min-h-[48px] sm:min-h-0"
            >
